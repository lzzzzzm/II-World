...
def process_one_sample(sem_pred, lidar_rays, output_origin, flow_pred):
    ...
    for t in range(T):
        ...
        pred_label = torch.from_numpy(sem_pred[coord_index[:, 0], coord_index[:, 1], coord_index[:, 2]])[:,
                     None]  # [N, 1]
        pred_dist = pred_dist[0, :, None].cpu()
        pred_pcds = torch.cat([pred_pcds[0], pred_label, pred_dist, pred_flow],
                              dim=-1)  # [N, 7]  5: [x, y, z, label, dist, dx, dy]
